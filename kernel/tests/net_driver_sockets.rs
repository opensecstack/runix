...
fn kernel_main(boot_info: &'static mut BootInfo) -> ! {
    ...
    let denied = unsafe {
        runix_kernel::syscall::syscall(
            ...
            SocketRequest::Close.encode()[0] as u64,
            0,
        )
    };
    if denied != u64::MAX {
...
fn run_socket_client() -> SocketTestResult {
    ...
    send_request(&SocketRequest::Send(TCP_PING.to_vec()));
    match recv_response() {
        Some(SocketResponse::Sent { len }) if len as usize == TCP_PING.len() => {}
        other => {
    ...
    for _ in 0..2000 {
        ...
        match recv_response() {
            Some(SocketResponse::Data(bytes)) => {
    ...
    send_request(&SocketRequest::Close);
    match recv_response() {
        Some(SocketResponse::Closed) => SocketTestResult::Pass,
        other => {
