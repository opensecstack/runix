//! Filesystem driver, Phase 1 (see docs/STATUS.md's "Filesystem driver,
//! Phase 1: virtio-blk transport" section): proves a real `blk-driver-host`
//! ring-3 process can write a sector to a real virtio-blk device and read
//! the exact bytes back. Structurally identical to `net_driver_icmp.rs`
//! (ELF-load, map heap/stack/queue/info pages, issue capability token,
//! spawn, poll the result byte) — see that file for why each step is shaped
//! the way it is.
//!
...
//!
//! Pass/fail is a real result code, not "didn't crash": `blk-driver-host`
//! writes a PASS/FAIL byte into the shared `BlkBootInfo` page after its own
//! write-then-read-back round trip on sector 0 finishes (see
//! `kernel/src/main.rs`'s `BLK_RESULT_OFFSET` doc comment) — reaching the
//! end of this test's yield budget without a fault only proves nothing
//! crashed; reading back an actual PASS byte proves the round trip through
//! the real virtio-blk device genuinely succeeded, with exact byte equality
//! verified.
...
// Must match `blk-driver-host/src/main.rs`'s `BLK_RESULT_OFFSET`/`BLK_RESULT_PASS`.
const BLK_RESULT_OFFSET: u64 = 128;
...
fn kernel_main(boot_info: &'static mut BootInfo) -> ! {
        ...
        .and_then(|dev| runix_kernel::pci::read_bar0_io_port(&dev))
    {
        ...
        None => {
            serial_println!(
                "blk_driver_rw: FAIL — no virtio-blk I/O-space BAR0 found (is xtask's \
    ...
    if let Err(e) = runix_kernel::citadel::demo_authorize(
        ...
        serial_println!(
            "blk_driver_rw: FAIL — CITADEL allowlist rejected blk-driver-host: {:?}",
            e
        );
        exit_qemu(QemuExitCode::Failed);
    }
    ...
    serial_println!(
        "blk_driver_rw: parsing blk-driver-host ({} bytes)",
        BLK_DRIVER_HOST_ELF.len()
    );
    let elf = match Elf64::parse(BLK_DRIVER_HOST_ELF) {
        Ok(elf) => elf,
        Err(e) => {
            serial_println!("blk_driver_rw: FAIL — parse() rejected the binary: {:?}", e);
            exit_qemu(QemuExitCode::Failed);
        }
    };
    ...
    let entry = match elf.load_segments(&mut space) {
        Ok(entry) => entry,
        Err(e) => {
            serial_println!("blk_driver_rw: FAIL — load_segments() failed: {:?}", e);
            exit_qemu(QemuExitCode::Failed);
        }
    };
    serial_println!("blk_driver_rw: loaded, entry point {:#x}", entry.as_u64());
    ...
    let result_ptr = unsafe { info_content.as_mut_ptr().add(BLK_RESULT_OFFSET as usize) };
    ...
    if result == BLK_RESULT_PASS {
        serial_println!(
            "blk_driver_rw: PASS — a real compiled binary wrote a sector to a real virtio-blk \
             device and read the exact bytes back, in an isolated ring 3 process"
        );
        exit_qemu(QemuExitCode::Success);
    } else {
        serial_println!(
            "blk_driver_rw: FAIL — blk-driver-host reported result byte {} (0 = never finished, \
             2 = round trip failed)",
            result
        );
        exit_qemu(QemuExitCode::Failed);
...
fn panic(info: &PanicInfo) -> ! {
    serial_println!("blk_driver_rw: PANIC: {}", info);
    exit_qemu(QemuExitCode::Failed);
}
